...
# Uses a hash of interface name to generate consistent table ID
mwan3nft_get_table_id() {
	local iface="$1"
	local id

	# Generate table ID from interface name (100-199 range)
	id=$(echo "$iface" | cksum | cut -d' ' -f1)
	id=$((100 + (id % 100)))
...
mwan3nft_port_to_nft_set() {
	...
	for port in $(mwan3nft_parse_port_list "$ports"); do
