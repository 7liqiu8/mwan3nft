...
mwan3nft_nft_build_lb_rules() {
	...
	for entry in $members; do
		...
		local prob_start=$prob_sum
		prob_sum=$((prob_sum + weight))

		# Use numgen random for weighted distribution
		# numgen random mod $total_weight gives 0 to (total_weight-1)
		if [ "$prob_start" -eq 0 ]; then
			nft add rule $NFT_FAMILY $NFT_TABLE "$chain" \
				"numgen random mod $total_weight < $prob_sum meta mark set $fwmark accept" 2>/dev/null
		else
			nft add rule $NFT_FAMILY $NFT_TABLE "$chain" \
				"numgen random mod $total_weight >= $prob_start numgen random mod $total_weight < $prob_sum meta mark set $fwmark accept" 2>/dev/null
		fi
	done
}
